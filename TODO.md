...
- [ ] **Store singleton** — `_store(cache_dir)` creates a new `Store` every call (mkdir, executescript, migrations). Cache the instance per `cache_dir` with `functools.lru_cache`. Significant on RPi.
- [ ] **Docker Compose tick-interval** — Poller tick is 30s but live targets are due every 25s. Reduce to `--tick-interval 5` (tick = how often worker checks, not fetch interval).
