...
- [x] **Thread safety for `_session`** — `requests.Session` is shared across threads if the API bootstrap path runs concurrently with the poller. Low risk but worth fixing.
